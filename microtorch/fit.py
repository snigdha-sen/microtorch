...
from utils.args import gen_args
...
def fit_model(args):
    ...
    param_map = np.zeros((*np.shape(mask),modelfunc.n_parameters + modelfunc.n_fractions))
    for i in range(0,modelfunc.n_parameters + modelfunc.n_fractions):
    ...
    zslice = 0
    _, ax = plt.subplots(1, modelfunc.n_parameters + modelfunc.n_fractions ,figsize=(5 * (modelfunc.n_parameters + modelfunc.n_fractions), 2))
    for i in range(0,modelfunc.n_parameters + modelfunc.n_fractions):
        im = ax[i].imshow(param_map[:, :, zslice, i])
        _  = plt.colorbar(im, ax=ax[i])
        ax[i].set_title(modelfunc.parameter_names[i] + ' (' + modelfunc.compartment_names[modelfunc.comp_ind[i]] + ')')
...
def main():
    ...
    parser.add_argument("-bd",  "--bdelta",     help="shape of gradient pulse", type = str, default = None)
    ...
    torch.autograd.set_detect_anomaly(True) 
    lossfunc = nn.MSELoss()
    net = Net(grad, modelfunc, layer_dims=grad.number_of_measurements, n_layers=3, dropout_fraction=args.dropout_frac, clipping_method=args.clip, activation=mlp_activation[args.activation])
    ...
    # Create folder to store results
    output_folder = "../contributor_folders/snigdha/results"
    Path(output_folder).mkdir(parents=True, exist_ok=True)  
    ...
    for i in range(0,modelfunc.n_parameters + modelfunc.n_fractions):
        im = ax[i].imshow(param_map[:, :, zslice, i])
        _  = plt.colorbar(im, ax=ax[i])
        ax[i].set_title(modelfunc.parameter_names[i] + ' (' + modelfunc.compartment_names[modelfunc.compartment_indices[i]] + ')')
