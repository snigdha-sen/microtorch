...
class Standard_WM: ## check with leon
    ...
    def __call__(self, grad, parameters):
        ...
        p00 = 1/torch.sqrt(torch.tensor(4)*torch.pi)*torch.ones_like(parameters[:,4].unsqueeze(1))

        S0 = parameters[:,0].unsqueeze(1)
        Di = parameters[:,1].unsqueeze(1)
        De = parameters[:,2].unsqueeze(1)
        Dp = parameters[:,3].unsqueeze(1)
        f  = parameters[:,4].unsqueeze(1)
        
        fODF = [p00,parameters[:,5].unsqueeze(1),parameters[:,6].unsqueeze(1),parameters[:,7].unsqueeze(1),parameters[:,8].unsqueeze(1),parameters[:,9].unsqueeze(1)]
