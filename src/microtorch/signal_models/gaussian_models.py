...
class Ballt2:
    ...
    def __init__(self):
        self.parameter_ranges = [[0.001, 3.0], [0.01, 0.5]]
...
class Tensor:
    ...
    def __call__(self, grad: AcquisitionScheme, parameters: torch.Tensor) -> torch.Tensor:
        ...
        # principal direction
        e1 = torch.stack([sintheta * cosphi, sintheta * sinphi, costheta], dim=1)

        # (u, v) are orthonormal basis vectors perpendicular to e1
        u = torch.stack([costheta * cosphi, costheta * sinphi, -sintheta], dim=1)

        v = torch.stack([-sinphi, cosphi, torch.zeros_like(sinpsi)], dim=1)
        ...
        D = lam1 * proj1 * proj1 + lam2 * proj2 * proj2 + lam3 * proj3 * proj3
