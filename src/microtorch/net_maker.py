...
class Net(nn.Module):
    def __init__(
        ...
        self.post_param_dropout = nn.Dropout(dropout_fraction) if dropout_fraction > 0 else None
    ...
    def forward(
        ...
        if (
            self.network_type == "dev_mlp"
        ):  # can make softmax mlp by choosing clipping method to be softmax
            params = F.softplus(params)

        if self.clipping_method_fraction == "softmax" and self.post_param_dropout is not None:
        ...
        if modelfunc.n_fractions > 1:
            fractions = fraction_squash(
                clipping_method_fraction, params[:, frac_start:frac_end], modelfunc, tau=1.0
            )
