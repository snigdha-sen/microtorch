...
def sphere2cart(theta: torch.Tensor, phi: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        n (torch.Tensor): A tensor of shape (3, B) containing the Cartesian coordinates
            corresponding to the input spherical coordinates.
    """
...
def cart2sphere(xyz: np.ndarray) -> np.ndarray:
    ...
    Returns:
        mu (torch.Tensor): A tensor of shape (..., 2) containing the spherical coordinates
            (theta, phi) corresponding to the input Cartesian coordinates.
    """
